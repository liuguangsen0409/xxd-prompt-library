<template>
  ...
  <LayoutBanner v-if="!isEmbed && config.banner.enable" />
  <LayoutHeader v-if="!isEmbed" />
  <NuxtPage v-if="isEmbed" />
  <div
    v-else-if="page && !page.fullpage && baseRouteName !== 'index'"
    class="min-h-screen"
...
const isEmbed = computed(() => route.path.replace(/\/$/, '') === '/embed/styles');
